...
| HLA & TCR motifs | synthetic fixture · real bulk TCRβ + real genotypes · real antigen-selected single cells | `build_hla_tcr_demo.R` · `build_hla_tcr_bulk_demo.R` · `build_hla_tcr_10x_demo.R` | self-contained (the two real ones download their sources on first run: ~349 MB from Zenodo, ~1.6 GB from 10x) | [`hla.md`](hla.md) |
...
| `vdj_10x_dextramer/` | **no** (cache, ~2.7 GB) | `build_hla_tcr_10x_demo.R` | the 10x dextramer cohort: per-donor contig annotations, binarized dextramer matrices, and the expression matrices |
